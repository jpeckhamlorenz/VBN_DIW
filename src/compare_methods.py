...
def process_one_method(
    ...
    print(f"    Distances: mean={signed_distances.mean():.4f}, "
          f"std={signed_distances.std():.4f} mm")
...
def print_comparison(results: dict[str, MethodResult]) -> None:
    ...
    for name in names:
        ...
        print(f"  {name}: total volumetric error = {total_vol_error:.4f} mm³ "
              f"(excess/deficit ratio = {ratio:.2f})")
...
def generate_figures(results: dict[str, MethodResult]) -> None:
    ...
    # KDE comparison
    plot_deviation_kde(all_distances, VIS_CONFIG,
                       output_path=OUTPUT_DIR / "kde_comparison.png")
    print(f"  -> {OUTPUT_DIR / 'kde_comparison.png'}")

    # Boxplot
    plot_boxplot_comparison(all_distances, VIS_CONFIG,
                            output_path=OUTPUT_DIR / 'boxplot_comparison.png')
    print(f"  -> {OUTPUT_DIR / 'boxplot_comparison.png'}")

    # Metrics table (PNG)
    plot_metrics_summary_table(all_metrics, VIS_CONFIG,
...
if __name__ == "__main__":
    ...
    print(f"  {len(np.asarray(cad_mesh_o3d.vertices)):,} vertices, "
          f"{len(np.asarray(cad_mesh_o3d.triangles)):,} triangles")

    # Process each method
    results: dict[str, MethodResult] = {}
    for display_name, scan_csv, toolpath_csv in METHODS:
        r = process_one_method(
            display_name, scan_csv, toolpath_csv,
            cad_mesh_o3d, cad_mesh_trimesh,
        )
