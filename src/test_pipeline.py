...
if RUN_STAGE_3_POINT_CLOUD:
    ...
    print(f"  Point cloud XYZ ranges:")
    for ax, label in enumerate(["X", "Y", "Z"]):
        print(f"    {label}: [{valid_points[:, ax].min():.3f}, {valid_points[:, ax].max():.3f}] mm")
    print(f"  Point area (res x slice_thickness): "
...
if RUN_STAGE_5_SEGMENT:
    ...
    bead_points, bed_points, bead_mask = segment_bed_from_bead(
    ...
    ax.scatter(bed_points[::step_bed, 0], bed_points[::step_bed, 1],
               c="lightgray", s=0.1, label="Bed", rasterized=True)
    ax.scatter(bead_points[::step_bead, 0], bead_points[::step_bead, 1],
               c="red", s=0.3, label="Bead", rasterized=True)
    ax.set_xlabel("X (mm)")
    ax.set_ylabel("Y (mm)")
    ...
    ax.scatter(bed_points[::step_bed, 0], bed_points[::step_bed, 2],
               c="lightgray", s=0.1, label="Bed", rasterized=True)
    ax.scatter(bead_points[::step_bead, 0], bead_points[::step_bead, 2],
               c="red", s=0.3, label="Bead", rasterized=True)
    ax.axhline(reg_config.bed_z_threshold, color="blue", linestyle="--",
               linewidth=1, label=f"Threshold = {reg_config.bed_z_threshold} mm")
    ax.set_xlabel("X (mm)")
    ax.set_ylabel("Z (mm)")
...
if RUN_STAGE_6_CAD_MESH:
    ...
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.scatter(cad_pts[:, 0], cad_pts[:, 1],
               c="blue", s=0.5, label="CAD mesh samples", alpha=0.5)
    step_bead = max(1, len(bead_points) // 20_000)
    ax.scatter(bead_points[::step_bead, 0], bead_points[::step_bead, 1],
               c="red", s=0.5, label="Bead points (pre-registration)", alpha=0.5)
    ax.set_xlabel("X (mm)")
    ax.set_ylabel("Y (mm)")
...
if RUN_STAGE_7_REGISTER:
    ...
    transform, fitness, inlier_rmse = register_scan_to_cad(
        bead_points, cad_mesh, reg_config
    )
    ...
    print(f"  Aligned bead XYZ ranges:")
    for ax_i, label in enumerate(["X", "Y", "Z"]):
    ...
    axes[0].scatter(bead_points[::step_bead, 0], bead_points[::step_bead, 1],
                    c="red", s=0.5, alpha=0.5, label="Bead (before)")
    ...
    axes[1].scatter(aligned_bead[::step_aligned, 0], aligned_bead[::step_aligned, 1],
                    c="red", s=0.5, alpha=0.5, label="Bead (after)")
...
if RUN_STAGE_8_DISTANCES:
    ...
    sc = ax.scatter(aligned_bead[::step, 0], aligned_bead[::step, 1],
                    c=signed_distances[::step], cmap="RdBu_r",
                    vmin=-clim, vmax=clim, s=0.3, rasterized=True)
    ax.set_xlabel("X (mm)")
    ax.set_ylabel("Y (mm)")
